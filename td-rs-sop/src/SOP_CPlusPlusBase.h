...
public:
  ...
  // Note that if you set this flag to 'true', then the function executeVBO() is
  // being called instead of execute() function. If this flag is set to false,
  // then execute() function is called. When directToGPU is true, the data is
  // not available to be used in SOPs, except to be rendered with a Render TOP.
  // DEFAULT: false.
  bool directToGPU;

  // Legacy code used clockwise vertex winding, but counter-clockwise is more
  // consistent with TouchDesigner. Older plugins will have this set to
  // LegacyCW, but newer ones should set it to CCW, as the sample code does.
  SOP_Winding winding;
...
public:
  ...
  // Note that has been the points must be already added by calling addPoints()
  // or addPoint(). The startPointIdx indicates the start index of the points to
  // set normals for.
  virtual bool setNormals(const Vector *n, int32_t numPoints,
                          int32_t startPointIdx) = 0;

  // Returns true if the normal has been set for this geometry.
  virtual bool hasNormal() = 0;

  // Set the color value with Color (i.e. r,g,b,a) for the point with 'pointIdx'
  // index. The point must already exist by via calling addPoints() or
  // addPoint().
  virtual bool setColor(const Color &c, int32_t pointIdx) = 0;

  // Set the colors for points that are already added.
  // The startPointIdx indicates the start index of the points to set colors
  // for.
  virtual bool setColors(const Color *colors, int32_t numPoints,
                         int32_t startPointIdx) = 0;
  ...
  // The numLayers is the texcoord size and can be from 1 up to 8 for texture
  // layers The first numLayers used will be used as the max number of layers
  // for all future calls for this cook. The pointIdx specifies the point index
  // with the texture coords
  virtual bool setTexCoord(const TexCoord *tex, int32_t numLayers,
                           int32_t pointIdx) = 0;

  // Set texture coordinate data for existing points.
  // The numLayers is the texCoord size and can be from 1 up to 8 for texCoord
  // layers. The first numLayers used will be used as the max number of layers
  // for all future calls for this cook. The startPointIdx indicates the start
  // index of the points to set texCoord for.
  virtual bool setTexCoords(const TexCoord *t, int32_t numPoints,
                            int32_t numLayers, int32_t startPointIdx) = 0;
  ...
  // Set the custom attribute with SOP_CustomAttribData (must have set its name,
  // number of components, and its type) The data param must hold the data for
  // the custom attribute. E.g a custom atrrib with 4 components for each point
  // should holds 4*numPoints values for its data.
  virtual bool setCustomAttribute(const SOP_CustomAttribData *cu,
                                  int32_t numPoints) = 0;
  ...
  // add particle systems from the points that has been already added. The
  // points can have colors, normals and custom attribs. the startIndex param is
  // the staring index of the points from particle system.
  virtual bool addParticleSystem(int32_t numParticles, int32_t startIndex) = 0;

  // Add line strip from the points that has been already added. The points can
  // have colors, normals and custom attribs. the 'indices' contains the indices
  // of vertices, and 'size' is the number of indices for the line strip
  virtual bool addLine(const int32_t *indices, int32_t size) = 0;

  // Add line strips from the points that has been already added.The points can
  // have colors, normals and custom attribs. the 'indices' contains the indices
  // of vertices, 'sizeOfEachLine' contains the number of vertices for each
  // line, 'numOfLines' specifies the number of lines to be drawn. Note that the
  // number of elements in sizeOfEachLine must be equal to numOfLines.
  virtual bool addLines(const int32_t *indices, int32_t *sizeOfEachLine,
                        int32_t numOfLines) = 0;

  // Returns the number of added primitives at the time of query. Currently it
  // is either the number of triangles or particles.
  ...
  // You may set this value at each frame for non static geometries that are
  ...
  // Returns false if a group with this name for the specified type does not
  // exists.
  virtual bool destroyGroup(const SOP_GroupType &type, const char *name) = 0;

  // Add a point with its index to an already existing group with
  // SOP_GroupType::Point type. Returns false if a point group with this name
  // does not exists Or if a point with that index does not exists.
  virtual bool addPointToGroup(int index, const char *name) = 0;

  // Add a primitive with its index to an already existing group with
  // SOP_GroupType::Primitive type. Returns false if a primitive group with this
  // name does not exists Or if a pritimive with that index does not exists.
  ...
  virtual bool addToGroup(int index, const SOP_GroupType &type,
                          const char *name) = 0;
  ...
  // Returns false if a primitive group with this name does not exists Or
  // if a pritimive/point with that index does not exists.
  virtual bool discardFromGroup(int index, const SOP_GroupType &type,
                                const char *name) = 0;
...
// This class is used to load geometry directly onto the GPU. The geometry can
// be used for rendering, but not SOP networks. NOTE: set 'directToGPU' flag
// from SOP_GeneralInfo class to true.
class SOP_VBOOutput {
...
public:
  ...
  // enable/set the normal, color, texcoord, if the geometry contains this
  // information
  ...
  // Returns true if the normal, color, texcoord, or custom attributes has been
  // set for this geometry.
  virtual bool hasNormal() = 0;
  ...
  // Add the custom attribute with SOP_CustomAttribInfo (must have set its name,
  // number of components, and its type)
  ...
  // to build primitives.
  virtual void allocVBO(int32_t numVertices, int32_t numIndices,
                        VBOBufferMode mode) = 0;
  ...
  // Returns an array of indices (of vertices) that should be filled. The
  // indices for all the triangles must be set as their initial values is
  // undefined. It is used to create 'numTriangles' triangles. Length of the
  // returned array is numTriangles * 3.
  virtual int32_t *addTriangles(int32_t numTriangles) = 0;

  // Returns an array of indices (of vertices) that should be filled. The
  // indices for particles must be set as their initial values is undefined. It
  // is used to create 'numParticles' particles. Length of the returned array is
  // numParticles.
  ...
  // The intData or floatData member contains a pointer to the member that
  // should be filled. Returns false is case of null arguments, or invalid name.
  virtual bool getCustomAttribute(SOP_CustomAttribData *cu,
                                  const char *name) = 0;
...
public:
  ...
  // For direct GPU loading (i.e. "directToGPU" is set to true) this function is
  // being called instead of execute(). Fill the VBO buffers with the geometry
  // data, obtained from your desired algorithm or files, such as points,
  // normals, colors, texcoord, triangles, and etc.
  virtual void executeVBO(SOP_VBOOutput *, const OP_Inputs *,
                          void *reserved1) = 0;
...
static_assert(offsetof(SOP_GeneralInfo, cookEveryFrame) == 0,
              "Incorrect Alignment");
static_assert(offsetof(SOP_GeneralInfo, cookEveryFrameIfAsked) == 1,
              "Incorrect Alignment");
static_assert(offsetof(SOP_GeneralInfo, directToGPU) == 2,
              "Incorrect Alignment");
