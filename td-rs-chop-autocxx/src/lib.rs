...
use td_rs_base_autocxx::OP_CHOPInput;
use crate::cxx::CHOP_Output;
...
pub struct ChopInput {
    input: *mut crate::cxx::OP_CHOPInput,
}

impl  ChopInput {
    /// Create a new `ChopInput` from a pinning reference to a
    /// `ChopInput`.
    pub fn new(input: *mut crate::cxx::OP_CHOPInput) -> ChopInput {
        Self { input }
    }

    /// Get the number of channels in the input buffer.
    pub fn num_channels(&self) -> u32 {
        unsafe { (*self.input).getNumChannels() as u32 }
    }
